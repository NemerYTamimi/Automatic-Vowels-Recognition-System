function [wts,binfrqs] = fft2melmx(nfft, sr, nfilts, width, minfrq, maxfrq, htkmel, alfa, constamp)
...
if nargin < 8;     alfa = 1;       end
if nargin < 9;     constamp = 0;   end
...
minmel = hz2mel(minfrq, htkmel,alfa);
maxmel = hz2mel(maxfrq, htkmel,alfa);
binfrqs = mel2hz(minmel+[0:(nfilts+1)]/(nfilts+1)*(maxmel-minmel), htkmel,alfa);

binbin = round(binfrqs/sr*(nfft-1));

for i = 1:nfilts
%  fs = mel2hz(i + [-1 0 1], htkmel,alfa);
  fs = binfrqs(i+[0 1 2]);
  % scale by width
